...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    open override func viewDidLoad() {
        ...
        // Note: Transition animation can be added here if needed for custom presentation

        if originalImage.size.width < 1 || originalImage.size.height < 1 {
            // Invalid image size - show error and dismiss
            let alert = UIAlertController(title: "Error", message: "Invalid image. Please select a valid image.", preferredStyle: .alert)
    ...
    open override func viewDidLayoutSubviews() {
        ...
        if let viewFrame = defaultCropperState?.viewFrame,
            ...
            if needReload {
                // Note: Future enhancement - reload while preserving crop box state
                needReload = false
                resetToDefaultLayout()
            }
        } else {
            // Note: Future enhancement - support multi-orientation without resetting layout
            resetToDefaultLayout()
