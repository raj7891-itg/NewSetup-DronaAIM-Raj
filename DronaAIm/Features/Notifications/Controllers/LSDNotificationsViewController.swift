//
//  LSDnotificationsViewController.swift
//  DronaAIm
//
...
enum LSNotificationSegmentType {
    case all, unread, actionNeeded
}

class LSDNotificationsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    private func loadNotifications() {
        Task {
            ...
            do {
                try await viewModel.fetchNotifications(limit: String(limit))
                setSegmentTitle()
    ...
    private func setSegmentTitle() {
        self.segmentController.setTitle(" All (\(viewModel.allCount))", forSegmentAt: 0)
        self.segmentController.setTitle(" Unread (\(viewModel.unreadCount))", forSegmentAt: 1)
        self.segmentController.setTitle("Action Needed (\(viewModel.actionNeededCount))", forSegmentAt: 2)
    }
    ...
    private func updateToolbarOptions() {
        ...
        let hide = UIBarButtonItem(title: "Hide", style: .plain, target: self, action: #selector(markAsHide))

        let unHide = UIBarButtonItem(title: "UnHide", style: .plain, target: self, action: #selector(markAsUnHide))

        let hideAll = UIBarButtonItem(title: "Hide All", style: .plain, target: self, action: #selector(markAsHideAll))
        ...
        bottomBar.items = [readAll, space, hideAll]
        
        if readItem != nil {
            bottomBar.items = [unRead, space, hide]
        } else if unReadItem != nil {
            bottomBar.items = [read, space, hide]
        }
//        else {
...
//        }
    ...
    @IBAction func showHiddenAction(_ sender: UIButton) {
        let hiddenVC = LSHiddenNotificationsViewController.instantiate(fromStoryboard: .driver)
    ...
    private func readApi(for notifications: [LSNotification], reloadToggle: Bool = true) {
        ...
        let messageIds = unReadNotifications.map({ $0.messageID }).compactMap({ $0 })
        Task {
            do {
                LSProgress.show(in: self.view)
    ...
    private func unReadApi(for notifications: [LSNotification], reloadToggle: Bool = true) {
        let readNotifications = notifications.filter { $0.isRead == true }
        let messageIds = readNotifications.map({ $0.messageID }).compactMap({ $0 })
        Task {
            do {
                LSProgress.show(in: self.view)
    ...
    @objc private func markAsHideAll() {
        let notifications = viewModel.allNotifications()
        var hiddenIds = UserDefaults.standard.messageIds
    ...
    @IBAction func segmentAction(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0 {
            viewModel.segmentType = .all
        } else if sender.selectedSegmentIndex == 1 {
            viewModel.segmentType = .unread
        } else if sender.selectedSegmentIndex == 2 {
            viewModel.segmentType = .actionNeeded
        }
        viewModel.filterBy(segmentType: viewModel.segmentType)
        self.tableView.reloadData()
    }
...
//              unHideAction.image = UIImage(systemName: "eye")
...
extension LSDNotificationsViewController: LSNotificationDetailsDelegate {
...
extension LSDNotificationsViewController: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let offsetY = scrollView.contentOffset.y
        let contentHeight = scrollView.contentSize.height
        let frameHeight = scrollView.frame.size.height

        if offsetY > contentHeight - frameHeight - 50 { // Adjust the threshold as needed
//            loadMoreData()
        }
    }
    ...
    private func loadMoreData() {
        ...
        if !viewModel.canLoadMore() {
            // Stop loader if no more data to load
            let indexPath = IndexPath(row: viewModel.numberOfRows(), section: 0)
            if let loadingCell = tableView.cellForRow(at: indexPath) as? LSLoadingCell {
